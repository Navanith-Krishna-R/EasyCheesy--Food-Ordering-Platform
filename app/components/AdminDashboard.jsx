    ...
    <div className="min-h-screen bg-gray-50 flex text-sm md:text-base font-sans antialiased text-slate-900">
      ...
      <aside
        ...
        <nav className="p-3 space-y-1">
          {["orders", "items", "categories"].map((tab) => (
            <button
              key={tab}
              onClick={() => {
                setActiveTab(tab);
              ...
              {tab === "orders" ? <ClipboardList size={18} /> : tab === "items" ? <LayoutGrid size={18} /> : <List size={18} />}
              <span className="capitalize">{tab}</span>
              {tab === "orders" && orders.filter((order) => order.status === "pending").length > 0 && (
                <span className="ml-auto rounded-full bg-orange-500 px-2 py-0.5 text-xs text-white">{orders.filter((order) => order.status === "pending").length}</span>
              )}
            </button>
          ))}
        </nav>
      ...
      <main className="flex-1 h-screen overflow-y-auto flex flex-col relative">
        <header className="sticky top-0 z-20 bg-white/90 backdrop-blur border-b border-gray-200 px-4 py-3 flex justify-between items-center md:hidden">
          ...
          {activeTab !== "orders" && <div className="flex items-center gap-2">
            <button
          ...
          {activeTab === "orders" ? (
            <div className="space-y-4">
              <div className="flex items-center justify-between px-4 md:px-0">
                <div><p className="text-xs font-bold uppercase tracking-widest text-orange-600">Kitchen queue</p><h2 className="text-2xl font-black">Customer order requests</h2></div>
                <button onClick={fetchAllData} className="rounded-full border border-slate-200 bg-white p-2 text-slate-600"><RefreshCw size={18} /></button>
              </div>
              {orders.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-slate-300 bg-white py-16 text-center text-slate-500">No orders have arrived yet.</div>
              ) : orders.map((order) => (
                <article key={order._id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div><p className="text-xs font-bold uppercase tracking-wider text-slate-400">Order #{order._id.slice(-6).toUpperCase()}</p><h3 className="mt-1 text-xl font-black">{order.customerName}</h3><p className="text-sm text-slate-500">{order.phone} • {order.customerEmail}</p></div>
                    <div className="text-right"><p className="text-2xl font-black text-orange-600">₹{order.total.toFixed(2)}</p><span className={`inline-flex rounded-full px-3 py-1 text-xs font-black uppercase ${order.status === "approved" ? "bg-emerald-100 text-emerald-700" : order.status === "rejected" ? "bg-rose-100 text-rose-700" : "bg-amber-100 text-amber-700"}`}>{order.status}</span></div>
                  </div>
                  <div className="mt-4 rounded-xl bg-slate-50 p-4 text-sm"><p className="font-bold text-slate-700">{order.items.map((item) => `${item.quantity}× ${item.name}`).join(" • ")}</p><p className="mt-2 text-slate-500">Deliver to: {order.address}</p>{order.note && <p className="mt-1 text-slate-500">Note: {order.note}</p>}</div>
                  {order.status === "pending" && (
                    <div className="mt-4 grid grid-cols-2 gap-3">
                      <button onClick={() => updateOrderStatus(order._id, "approved")} className="flex items-center justify-center gap-2 rounded-xl bg-emerald-600 py-3 font-black text-white transition hover:bg-emerald-700"><Check size={18} /> Approve</button>
                      <button onClick={() => updateOrderStatus(order._id, "rejected")} className="flex items-center justify-center gap-2 rounded-xl bg-rose-600 py-3 font-black text-white transition hover:bg-rose-700"><Ban size={18} /> Reject</button>
                    </div>
                  )}
                </article>
              ))}
